...
function __OriginkitBase_SlideFillButton(props) {
  ...
  const fill = colors?.fill ?? fillProp ?? "#E6F8FD";
  const textColor = colors?.textColor ?? textColorProp ?? "#000000";
  const {
    color: waterColor = colors?.hoverFill ?? "#002E5B",
    textColor: waterTextColor = colors?.hoverTextColor ?? "#FFFFFF",
    direction = directionLegacy ?? "up",
    defaultFill = defaultFillLegacy ?? 3,
    waveSpeed: waveSpeedPctProp,
  } = water;

  const waveSpeedPct = waveSpeedPctProp ?? waveSpeedLegacy ?? 60;
  const waveSpeed =
  ...
  return (
    <Root
      ...
      style={{
        ...
        gap: (hasIcon || children) && showText ? gapPx : 0,
        flexDirection: iconSide === "right" ? "row-reverse" : "row",
        minWidth: 80,
        minHeight: 38,
        padding,
        borderRadius: radiusPx || 12,
        border: border?.borderWidth ? `${border.borderWidth}px ${border.borderStyle || 'solid'} ${border.borderColor || '#BAE6FD'}` : '1px solid #BAE6FD',
        background: fill,
        cursor: "pointer",
        textDecoration: "none",
        boxShadow: "0 2px 10px rgba(32, 183, 227, 0.2)",
    ...
    >
      ...
      {/* Children or Custom Icon if provided */}
      {children && (
        <motion.span
          ref={iconRef}
          style={{
